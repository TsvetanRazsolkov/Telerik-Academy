...
namespace _02.JediMeditation
{
    ...
    public class Program
    {
        public static void Main()
        {
            ...
            Dictionary<char, List<string>> jediOrder = new Dictionary<char, List<string>>();


            jediOrder.Add('m', new List<string>());
            jediOrder.Add('k', new List<string>());
            jediOrder.Add('p', new List<string>());

            foreach (var jed in jedis)
            {
                char key = jed[0];

                jediOrder[key].Add(jed);
            }

            Console.Write(string.Join(" ", jediOrder['m']));
            if (jediOrder['m'].Count > 0)
            {
                Console.Write(' ');
            }

            Console.Write(string.Join(" ", jediOrder['k']));
            if (jediOrder['k'].Count > 0)
            {
                Console.Write(' ');
            }

            Console.Write(string.Join(" ", jediOrder['p']));
        }
    }
}
