...
function solve(){
  return function (animals) {
      ...
      _.chain(animals).groupBy('species')
          .sortBy('species')
          .reverse()
          .map(function(group){
              return _.chain(group).sortBy('name')
                                    .sortBy('legsCount')
                                    .value();
          })
          .each(function(group){
              var groupName = group[0].species,
                  separatorLength = groupName.length + 2,
                  separator = new Array(separatorLength).join('-');
